...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *_hcan)
{
    ...
    {			
        ...
        else
        {
            ...
            case 0x150:
                CAP_vol = rx_data[0];
                ina226[0].Bus_Voltage = (float)CAP_vol / 5.0f;
                ina226[0].state=(float)rx_data[1];
                ina226[0].buck_duty=(float)rx_data[2];
                ina226[0].powerchassis=(float)((rx_data[3]<<8|rx_data[4])/10.0f);
                count_CAN[1]++;
                Detect_Hook(CAP_TOE);
                break;
